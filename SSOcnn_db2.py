...
DATA_FILE_NAME = "Data_for_ML_TT-DB2-2"
TIME_STEP = 10
Y_COL = 2
FEATURE_NUMBER = 3
...
def ORIGIN(X):  # _init
    ...
    X[0], X[1] = 32, 1  # filter number & size
    X[2] = 1  # strides
    X[3] = 2  # pool_size
    X[4], X[5] = 32, 2  # filter number & size
    X[6] = 1  # strides
    X[7] = 1  # pool_size
    X[8] = 100  # input_layer
...
def VAR_init(var, X, Y):  # _init
    if var in [0]:
        ...
        X[var] = random.randint(2, min(10, Y[5]))  # filter step
...
def VAR_check(var, X, Y):  # _init
    if var in [0, 1, 2, 4, 6, 8]:
        return  # filter number, filter size, strides first conv layer

    elif var in [3]:
        y1, y2, x1, x2 = 0, 5, 1, 2  # pooling X-size first pooling layer
    elif var in [5]:
        y1, y2, x1, x2 = 1, 0, 3, 3  # filter X-size second conv layer
    elif var in [7]:
        y1, y2, x1, x2 = 2, 1, 5, 6  # pooling X-size second pooling layer
...
if __name__ == "__main__":
    ...
    for run in range(0, Nrun):
        ...
        # ...............................
        f = open(f"./SSO_output/SSO_cnn_{DATA_FILE_NAME}.txt", "a+")
