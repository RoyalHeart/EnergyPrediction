...
def LeNet(X):
    ...
    # model.add(Conv1D(X[0], (int(X[1])), activation="relu", input_shape=(Nx, Ny)))
    # model.add(LSTM(X[2], "relu", return_sequences=True))
    # model.add(LSTM(X[3], "relu"))
    # model.add(Dense(X[4]))
    # model.add(Dense(X[5]))
    model.add(Dense(1))
...
def ORIGIN(X):  # _init
    ...
    X[0], X[1], X[2] = 32, 1, 1  # filter number & size
    X[3], X[4] = 1, 1  # strides
    X[5], X[6] = 2, 2  # pool_size
    X[7], X[8], X[9] = 32, 2, 2  # filter number & size
    X[10], X[11] = 1, 1  # strides
    X[12], X[13] = 1, 1  # pool_size
    X[14] = 100  # input_layer
    # X[15] = 10                     #hidden_layer
...
def VAR_init(var, X, Y):  # _init
    if var in [0]:
        ...
        X[var] = random.randint(1, 100)  # filter number

    elif var in [1]:
        X[var] = random.randint(2, min(10, Y[8]))  # filter step
    elif var in [2]:
        X[var] = random.randint(2, min(10, Y[9]))  # filter step
        ...
        X[var] = random.randint(2, min(4, Y[0] + 2))  # pooling X-size
    elif var in [6]:
        X[var] = random.randint(2, min(4, Y[1] + 2))  # pooling Y-size

    elif var in [8]:
        X[var] = random.randint(2, min(4, Y[2] + 2))  # filter X-size
    elif var in [9]:
        X[var] = random.randint(2, min(4, Y[3] + 2))  # filter Y-size
        ...
        X[var] = random.randint(2, min(4, Y[4] + 2))  # pooling X-size
    elif var in [13]:
        X[var] = random.randint(2, min(4, Y[5] + 2))  # pooling Y-size

    elif var in [14]:
        X[var] = random.randint(1, 1000)  # input size
...
def VAR_check(var, X, Y):  # _init
    if var in [0, 1, 2, 3, 4, 7, 10, 11, 14]:
        return  # filter number, filter size, strides first conv layer

    elif var in [5]:
        y1, y2, x1, x2 = 0, 8, 1, 3  # pooling X-size first pooling layer
    elif var in [6]:
        y1, y2, x1, x2 = 1, 9, 2, 4  # pooling Y-size first pooling layer
    elif var in [8]:
        y1, y2, x1, x2 = 2, 0, 5, 5  # filter X-size second conv layer
    elif var in [9]:
        y1, y2, x1, x2 = 3, 1, 6, 6  # filter Y-size second conv layer
    elif var in [12]:
        y1, y2, x1, x2 = 4, 2, 8, 10  # pooling X-size second pooling layer
    elif var in [13]:
        y1, y2, x1, x2 = 5, 3, 9, 11  # pooling Y-size second pooling layer
...
if __name__ == "__main__":
    Ngen, Nsol, Nvar, Nrun = 100, 5, 15, 1
